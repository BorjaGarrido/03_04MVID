...
glm::vec3 teatPos(0.0f, 0.0f, 0.0f);
...
void render(float x, float y, const Geometry& object, const Geometry& floor, const Geometry& light, const Shader& s_phong, const Shader& s_light) {
    ...
    s_phong.use();
    model = glm::mat4(1.0f);
    model = glm::translate(model, teatPos);
    model = glm::translate(model, glm::vec3(0.0f, -0.5f, 0.0f));
    model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
...
int main(int, char* []) {
    ...
    while (window->alive()) {
        ...
        render(x, y, teapot, quad, sphere, s_phong, s_light);
        window->frame();
