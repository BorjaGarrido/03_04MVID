...
glm::vec3 lightPos(3.0f, 6.0f, -2.0f);
glm::vec3 lightPos2(-3.0f, 6.0f, -2.0f);
...
void renderScene(const Shader& shader, const Geometry& quad, const Geometry& cube, const Geometry& sphere,
    const Geometry& light, const Shader& s_light, const Texture& t_albedo, const Texture& t_specular) {
...
void render(const Geometry& quad, const Geometry& cube, const Geometry& sphere, const Geometry& light,
    ...
    const glm::mat4 lightProjection = glm::perspective<float>(glm::radians(45.0f), 1.0f, 2.0f, 20.0f);
    ...
    renderScene(s_depth, quad, cube, sphere, light, s_light,t_albedo, t_specular);
    ...
    renderScene(s_phong, quad, cube, sphere, light,  s_light, t_albedo, t_specular);
    ...
    const glm::mat4 lightProjection2 = glm::perspective<float>(glm::radians(45.0f), 1.0f, 2.0f, 20.0f);

    const glm::mat4 lightView2 = glm::lookAt(lightPos2, glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    //Si se hace el clear, al limpiar el buffer la segunda sombra si se dibuja y la primera NO, lógico
    //La segunda sombra por lo tanto si se puede dibujar, pero no ambas a la vez
    //glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    view = camera.getViewMatrix();
    proj = glm::perspective(glm::radians(camera.getFOV()), static_cast<float>(Window::instance()->getWidth()) / Window::instance()->getHeight(), 0.1f, 100.0f);
    ...
    s_phong.set("light.position", lightPos2);
    s_phong.set("light.ambient", 0.1f, 0.1f, 0.1f);
    s_phong.set("light.diffuse", 0.5f, 0.5f, 0.5f);
    s_phong.set("light.specular", 1.0f, 1.0f, 1.0f);
    s_phong.set("light.constant", 1.0f);
    ...
    s_phong.set("lightSpaceMatrix", lightSpaceMatrix2);
...
int main(int, char* []) {
    ...
    const Sphere sphere2(1.0f, 50, 50);
    ...
    while (window->alive()) {
        ...
        render(quad, cube, sphere, sphere2, s_phong, s_depth, s_debug, s_light, t_albedo, t_specular, fbo.first, fbo.second);
        window->frame();
    }
