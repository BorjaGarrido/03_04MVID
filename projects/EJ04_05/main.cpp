...
float handleInput(float interpos) {
    std::vector<std::pair<int, int>> keys = Input::instance()->getKeys();
    for (auto& key : keys) {
        std::cout << key.first << " - " << key.second << std::endl;
        if (key.first == 265 && interpos < 1.0) {
            interpos = interpos + 0.1;
        }
        else if (key.first == 264 && interpos > 0.0) {
            interpos = interpos - 0.1;
        }
    }
    return interpos;
...
void render(uint32_t VAO, const Shader& shader, uint32_t tex1, uint32_t tex2, float interpo) {
...
int main(int, char* []) {
    ...
    while (window->alive()) {
        interpos = handleInput(interpos);
        render(VAO, shader, tex1, tex2, interpos);
        window->frame();
    }
