...
uniform Light light[NUMBER_SPOT_LIGHTS];
...
vec3 calcSpotLight(Light light, vec3 normal, vec3 viewDir, vec3 fragPos, vec3 albedoMap, vec3 specularMap) {
    float distance = length(light.position - fragPos);
    float attenuation = 1.0 / (light.constant +
        light.linear * distance +
        light.quadratic * distance * distance);
...
void main() {
    ...
    for (int i = 0; i < NUMBER_SPOT_LIGHTS; ++i) {
        finalColor += calcSpotLight(light[i], norm, viewDir, fragPos, albedo, specular);
	}
