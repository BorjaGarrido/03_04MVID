...
glm::vec3 teaPos(0.0f, 0.0f, 0.0f);
...
void render(float x, float y, const Geometry& floor, const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light,
    ...
    for (uint32_t i = 0; i < sizeof(pointSpotLightPositions); ++i) {
        if (i == 0) {
            pointSpotLightPositions[0].x = x;
            pointSpotLightPositions[0].y = y;
        }
        else {
            pointSpotLightPositions[1].z = x;
            pointSpotLightPositions[1].y = y;
        }
        
    }

    for (const auto& pointPos : pointSpotLightPositions) {
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, pointPos);
        model = glm::scale(model, glm::vec3(0.25f));
        s_light.set("model", model);
    ...
    const std::string spotPoints = "light[";
    for (uint32_t i = 0; i < sizeof(pointSpotLightPositions); ++i) {
        const std::string lightName = spotPoints + std::to_string(i) + "].";

        s_phong.set((lightName + "position").c_str(), pointSpotLightPositions[i]);
        s_phong.set((lightName + "direction").c_str(), lightDir);
        s_phong.set((lightName + "ambient").c_str(), 1.0f, 0.1f, 0.1f);
        s_phong.set((lightName + "diffuse").c_str(), 0.5f, 0.0f, 0.8f);
        s_phong.set((lightName + "specular").c_str(), 0.0f, 1.0f, 1.0f);
        s_phong.set((lightName + "constant").c_str(), 1.0f);
        s_phong.set((lightName + "linear").c_str(), 0.2f);
        s_phong.set((lightName + "quadratic").c_str(), 0.06f);
        s_phong.set((lightName + "cutOff").c_str(), glm::cos(glm::radians(40.0f)));
        s_phong.set((lightName + "outerCutOff").c_str(), glm::cos(glm::radians(50.0f)));
    }
    ...
    floor.render();
        ...
        model = glm::mat4(1.0f);
        model = glm::translate(model, teaPos);
        model = glm::translate(model, glm::vec3(0.0f, -0.5f, 0.0f));
        model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
...
int main(int, char* []) {
    ...
    while (window->alive()) {
        ...
        float x = 3.0 * cos(grados);

        float y = 3.0 * sin(grados);

        grados++;

        handleInput(deltaTime);
        render(x, y, quad, teapot, sphere, s_phong, s_light, t_albedo, t_specular);
        window->frame();
    }
